...
export default function ProductCardDetail() {
  ...
  return <div className="mx-auto max-w-3xl py-8"><Card className="overflow-hidden"><img className="h-72 w-full object-cover sm:h-96" src={image} alt={data.name} onError={() => setFailed(true)} /><div className="space-y-3 p-6"><div className="flex items-center justify-between gap-3"><h1 className="text-3xl font-bold">{data.name}</h1><span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-200">{data.badge || 'Featured'}</span></div><div className="flex flex-wrap items-center gap-2"><span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${hasStock ? (lowStock ? 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-200' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-200') : 'bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-200'}`}>{hasStock ? (lowStock ? `Only ${stock} left in stock` : `In stock: ${stock}`) : 'Out of stock'}</span><span className="text-sm text-slate-500">MMK {Number(data.price ?? 0).toFixed(2)}</span></div><Button variant="outline" onClick={() => navigate('/')}>Back to catalog</Button><p className="text-slate-600 dark:text-slate-300">{data.description || 'A detailed view of this product.'}</p><p className="text-xs text-slate-500">{data.createdAt ? new Date(data.createdAt).toLocaleDateString() : 'Recently added'}</p></div></Card></div>;
}
