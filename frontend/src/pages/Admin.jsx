...
export default function Admin() {
  ...
  const errorMessage = productEditError?.message || flagsError?.message || productCardsError?.message || createMutation.error?.message || updateMutation.error?.message || removeMutation.error?.message || deleteUserMutation.error?.message || productCardMutation.error?.message || deactivateMutation.error?.message || deleteProductMutation.error?.message;
  const closeProductModal = () => { setOpenProductCard(false); setSelectedProductCard(null); setProductCardDraft(emptyCardDraft); };
  const openProductForEdit = async (item) => {
    setProductEditError(null);
    try {
      const response = await productApi.getProduct(item.id);
      const product = response?.data ?? response;
      setSelectedProductCard(product.id);
      setProductCardDraft({ ...emptyCardDraft, ...product, description: product.description || '', brand: product.brand || '', category: product.category || '', slug: product.slug || '', badge: product.badge || '', image: product.image || '', price: product.price ?? '', originalPrice: product.originalPrice ?? '' });
      setOpenProductCard(true);
    } catch (error) {
      setProductEditError(error);
    }
  };
  return <div className="py-4">
    ...
    <section className="mb-10"><div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-2xl font-bold">Product cards</h2><p className="text-sm text-slate-500">Manage prices, stock, images, brands, categories, and status.</p></div><Button onClick={() => { setSelectedProductCard(null); setProductCardDraft(emptyCardDraft); setOpenProductCard(true); }}>New product card</Button></div>
      {productCardsLoading ? <Spinner /> : <div className="grid gap-4 md:grid-cols-2">{productCards.map((item) => <Card key={item.id} className="p-4"><div className="flex justify-between gap-3"><h3 className="font-bold">{item.name}</h3><span className={`rounded-full px-2 py-1 text-xs font-semibold ${item.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{item.isActive ? 'Active' : 'Inactive'}</span></div><p className="mt-2 text-sm text-slate-500">${Number(item.price).toFixed(2)} · Stock: {item.stock} · {item.brand || 'No brand'}</p><div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" className="px-3 py-1.5" onClick={() => openProductForEdit(item)}>Edit</Button>{item.isActive ? <Button variant="dangerOutline" className="px-3 py-1.5" onClick={() => deactivateMutation.mutate(item.id)}>Deactivate</Button> : null}<Button variant="dangerOutline" className="px-3 py-1.5" onClick={() => { if (window.confirm(`Delete "${item.name}" permanently?`)) deleteProductMutation.mutate(item.id); }}>Delete</Button></div></Card>)}</div>}
    </section>
